...
try:
    ...
    if WITH_SEGMM and 'pytest' in sys.modules and 'torch' in sys.modules and torch.cuda.is_available():
        # NOTE: `segment_matmul` is currently bugged on older NVIDIA cards which
        # cause our GPU tests on CI to crash. Try if this error is present on the
